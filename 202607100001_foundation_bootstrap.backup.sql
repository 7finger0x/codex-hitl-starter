...
create role platform_owner with
  nologin
  inherit
  nosuperuser
  nocreatedb
...
create role platform_migrator with
  nologin
  noinherit
  nosuperuser
  nocreatedb
...
create role platform_runtime with
  nologin
  noinherit
  nosuperuser
  nocreatedb
...
create role platform_worker with
  nologin
  noinherit
  nosuperuser
  nocreatedb
...
create role platform_evidence_reader with
  nologin
  noinherit
  nosuperuser
  nocreatedb
...
create table platform_private.foundation_migration_ledger (
  ...
  constraint foundation_migration_ledger__migration_id_shape
    check (
      ...
      and migration_id ~ '^[0-9]{12}_[a-z][a-z0-9_]*$'
    ),
  ...
  constraint foundation_migration_ledger__tool_version_shape
    check (char_length(tool_version) between 1 and 128)
);
...
as $function$
  select nullif(current_setting('platform.user_id', true), '')::uuid
$function$;
...
set search_path = pg_catalog
as $function$
  select nullif(current_setting('platform.principal_id', true), '')::uuid
$function$;
...
as $function$
declare
  setting_value text := nullif(current_setting('platform.principal_type', true), '');
begin
...
set search_path = pg_catalog
as $function$
  select nullif(current_setting('platform.environment_id', true), '')::uuid
$function$;
...
declare
  setting_value text := nullif(
    current_setting('platform.authentication_strength', true),
    ''
  );
begin
  if setting_value is null then
...
set search_path = pg_catalog
as $function$
  select nullif(current_setting('platform.correlation_id', true), '')::uuid
$function$;
...
begin
  ...
  perform set_config('platform.user_id', p_user_id::text, true);
  perform set_config('platform.principal_id', p_principal_id::text, true);
  perform set_config('platform.principal_type', p_principal_type, true);
  perform set_config('platform.tenant_id', p_tenant_id::text, true);
  perform set_config('platform.environment_id', p_environment_id::text, true);
  perform set_config(
    'platform.authentication_strength',
    p_authentication_strength,
    true
  );
  perform set_config('platform.correlation_id', p_correlation_id::text, true);
end;
$function$;
...
as $function$
begin
  perform set_config('platform.user_id', '', true);
  perform set_config('platform.principal_id', '', true);
  perform set_config('platform.principal_type', '', true);
  perform set_config('platform.tenant_id', '', true);
  perform set_config('platform.environment_id', '', true);
  perform set_config('platform.authentication_strength', '', true);
  perform set_config('platform.correlation_id', '', true);
end;
$function$;
...
as $function$
  select candidate_tenant_id is not null
    and platform.current_tenant_id() is not null
...
as $function$
  select candidate_tenant_id is not null
    ...
    and platform.current_tenant_id() is not null
    and platform.current_environment_id() is not null
    and candidate_tenant_id = platform.current_tenant_id()
...
begin
  if tg_table_name = 'policy_rules' then
    policy_tenant_id := (old_document ->> 'tenant_id')::uuid;
    policy_environment_id := (old_document ->> 'environment_id')::uuid;
    policy_version_id := (old_document ->> 'policy_version_id')::uuid;
...
begin
  row_tenant_id := (new_document ->> 'tenant_id')::uuid;
  row_environment_id := (new_document ->> 'environment_id')::uuid;
